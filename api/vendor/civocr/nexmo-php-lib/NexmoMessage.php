...
class NexmoMessage {
	...
	var $nx_uri = 'https://rest.nexmo.com/sc/us/alert/json?';
	...
	function sendText ( $to, $from, $message, $unicode=null ) {
		...
		if ($message['safe'] != 'welcome'){
			$post['name']= $message['name'];
			$post['message'] = $message['message'];
            if ($message['safe'] != 'contact')
			    $post['phone']= $message['phone'];
		}
	...
	//	echo "<pre>";print_r($data);echo "</pre>";exit;
		...
		if (function_exists('curl_version')) {
			...
			$to_nexmo = curl_init( $this->nx_uri. http_build_query($data) );
